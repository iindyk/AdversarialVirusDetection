...
print('classifier2: initiated, error on test dataset is ', err_test1)
...
print('classifier4: initiated, error on test dataset is ', err_test2)
...
for i in range(len(steps)-1):
    ...
    for j in range(len(classifiers)):
        if classifiers[j].is_valid(new_infected_data, new_train_labels) and j != 3:
    ...
    classifiers[3].partial_fit(new_train_dataset, new_train_labels)

    if classifiers[0].is_valid(new_infected_data, new_train_labels):
        adversary.eps *= 1.1
    else:
        adversary.eps *= 0.9
