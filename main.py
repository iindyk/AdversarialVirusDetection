...
err_orig = 1 - accuracy_score(train_labels, svm_orig.predict(train_dataset))
adversary = adv.Adversary(None, None, train_dataset, train_labels, len(train_dataset[0]))
...
err_infected = 1 - accuracy_score(train_labels, svm_infected.predict(train_dataset))
...
for i in range(int(0.5*n)):
    if train_labels[i] == 1:
        ...
        colors.append((0, 0, 1))
    if np.linalg.norm(train_dataset_infected[i]-train_dataset[i]) > 0.1:
        inf_points.append(train_dataset_infected[i])
